...
export async function startValidationDb() {
  const state = {
  ...
  const server = createServer(async (request, response) => {
    ...
    if (table !== "product_facts" || request.method !== "PATCH") return fail();
    if (state.beforePatch) await state.beforePatch(payload);
    const status = payload.validation?.attempt.status;
    if (state.failure === `patch-${status}`) return fail();
    if (!state.facts || !matches(state.facts)) return send([]);
    Object.assign(state.facts, payload, { updated_at: new Date().toISOString() });
    if (state.ackLost === status) { state.ackLost = null; return fail(); }
    return send([state.facts]);
  });

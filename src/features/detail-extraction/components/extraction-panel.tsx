...
import { derivationSchema, isExtractionActive, isSaveActive, readExtraction, type ExtractionResult } from "../schemas";
...
function CandidateReview({ result, ...props }: Props & { result: ExtractionResult }) {
  ...
  const shown = result.candidates.filter(c => c.saveAllowed || showExcluded);
  async function save() {
  ...
  return <div className="mt-5 space-y-5">
    ...
    <ul className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
      {shown.map(candidate => { const number = result.candidates.indexOf(candidate) + 1, rect = candidate.rect, saved = existing.has(candidate.id);
        return <li key={candidate.id} className="min-w-0 rounded-lg border border-zinc-200 p-3">
          ...
          <label className="mt-3 flex items-start gap-2 text-sm font-medium"><input type="checkbox" className="mt-1" disabled={props.busy || saved || stale || !candidate.saveAllowed}
            ...
            {number}. {REGION_LABELS[candidate.regionType]}{saved ? " · 저장됨" : !candidate.saveAllowed ? " · 제외" : ""}</label>

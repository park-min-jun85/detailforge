...
export async function readEditRequest(request: Request, maxBytes = 32000) {
  try { assertSameOrigin(request); } catch { throw new EditorError("forbidden"); }
  if (!request.body || Number(request.headers.get("content-length")) > maxBytes) throw new EditorError("invalid_input");
  ...
  try {
    while (true) { const chunk = await reader.read(); if (chunk.done) break; size += chunk.value.length;
      if (size > maxBytes) { await reader.cancel(); throw new EditorError("invalid_input"); } body += decoder.decode(chunk.value, { stream: true }); }

...
import { optionCandidateSchema } from "@/features/product-options/schemas";
...
export const candidateSchema = z.object({ sourceType: z.literal("wholesale_url"), sourceUrl: remoteUrlSchema, sourceHost: z.string().max(253), fetchedAt: z.iso.datetime(), extractionMethod: extractionMethodSchema,
  product: extractedProductSchema, options: optionCandidateSchema.optional(), images: z.array(z.object({ url: remoteUrlSchema, alt: z.string().max(200).nullable(), selected: z.boolean() }).strict()).max(30), warnings: z.array(z.string().max(300)).max(10) }).strict();

...
export default async function PlannerPage({ params }: PageProps<"/projects/[projectId]/planner">) {
  ...
  return <div className="page-content">
    ...
    <ol aria-label="상세페이지 제작 단계" className="flex flex-wrap gap-x-6 gap-y-3 border-b border-zinc-200 pb-5 text-sm">
      ...
      <li aria-current="step" className="font-semibold">5. 페이지 설계</li><li className="text-zinc-500">6. 상세페이지</li>
    </ol>
    {result.ok ? <PlannerManager initialView={result.view} /> : result.error.code === "product_required" || result.error.code === "facts_required" ? <section className="panel space-y-4 p-8">

...
- Editor는 최신 읽기/저장/순서 복구/AI 생성/후보 적용/옵션 반영 진행 문구를 구분한다. 저장 후 최신 내용을 읽는 단계도 읽기 상태로 표시한다. 기존 busy 차단과 draft 확인 흐름은 유지한다.
- 최신 읽기 실패 시 draft를 보존하며 ‘최신 섹션 다시 불러오기’로 재시도하도록 안내한다. 실패 후 다시 읽기 또는 확인창 취소를 선택할 수 있다.
- 긴 이미지 부분 분석 실패 화면은 현재 성공 후보 저장 가능, 재분석은 전체 구간 AI 재호출이라는 범위/비용, 전체 실패 시 이전 성공 후보·저장 이미지 보존을 안내한다. 실패 구간 전용 재시도 CTA는 없다.
- 기존 보고체 문구의 검토 경고는 Editor/Final 준비 영역에만 표시한다. canonical 문구·레이아웃·Export 표면을 자동 수정하지 않는다. 여백·crop framing은 backlog에 유지한다.

## TASK-032 여정 보완

상품 저장 성공 시 옵션 가져오기 가능 상태가 즉시 갱신된다. 기존 상품의 입력 중인 옵션은 그대로 유지하며 상품 저장 성공이 옵션 저장 성공을 뜻하지 않는다. 이미지 화면의 다음 단계 안내는 실제 구현된 상품 분석→사실 검증→페이지 설계로 연결한다.

같은 원본/bytes/영역의 제품컷은 역할이 달라도 저장됨으로 표시한다. 선택 후보 수와 실제 신규 이미지 수를 구분하고 남은 슬롯 검사는 신규 고유 영역 기준이다. 추출 결과가 있으면 기존 후보를 바로 보여주고 재분석 버튼에 AI 재호출임을 명시한다.

QA에서 Editor 문구·스타일 저장/순서 저장/dirty 이동/오래된 탭 conflict 복구,375px 패널 전환과 키보드 포커스를 확인했다. Final은860px 고정이고 검토 경고와 다운로드 UI는 capture article 밖에 둔다. 상세 범위와 수동 확인 항목은 [TASK-032](tasks/TASK-032.md).
...
## TASK-021B 도매매 옵션 가져오기

OptionsManager에서 저장된 원본 URL/상품번호 → 도매매 옵션 불러오기 → 이전 원본/현재 저장값/새 후보 비교 → 신규 값 체크 → 입력란 반영 → 별도 옵션 저장을 제공한다.
